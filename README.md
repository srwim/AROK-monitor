...
Other launchers: **`run_desktop.bat`** — native window (pywebview/WebView2), no browser. **`make_installer.bat`** — full pipeline: React build → PyInstaller `AROK.exe` → Inno Setup `installer_out\AROK-Setup-1.1.1.exe` (needs Inno Setup 6).
